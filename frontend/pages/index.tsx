...
const getUnfinishedVideoTheme = (subspecialtyLabel: string) => {
  ...
  if (normalized.includes('rhin')) {
    return {
      ...
      cardClass: 'border-[color-mix(in_oklab,var(--specialty-rhinology)_36%,var(--app-border)_64%)] bg-[linear-gradient(180deg,color-mix(in_oklab,var(--specialty-rhinology)_14%,var(--app-surface)_86%)_0%,color-mix(in_oklab,var(--specialty-rhinology)_24%,var(--app-surface-alt)_76%)_100%)]',
      stripeClass: 'bg-[linear-gradient(90deg,color-mix(in_oklab,var(--specialty-rhinology)_84%,#0ea5e9_16%),color-mix(in_oklab,var(--specialty-rhinology)_68%,#2563eb_32%))]',
      ctaClass: 'text-[color-mix(in_oklab,var(--specialty-rhinology)_78%,var(--app-text)_22%)]',
    };
  }
  ...
  if (normalized.includes('lary')) {
    return {
      ...
      cardClass: 'border-[color-mix(in_oklab,var(--specialty-laryngology)_36%,var(--app-border)_64%)] bg-[linear-gradient(180deg,color-mix(in_oklab,var(--specialty-laryngology)_14%,var(--app-surface)_86%)_0%,color-mix(in_oklab,var(--specialty-laryngology)_24%,var(--app-surface-alt)_76%)_100%)]',
      stripeClass: 'bg-[linear-gradient(90deg,color-mix(in_oklab,var(--specialty-laryngology)_84%,#ec4899_16%),color-mix(in_oklab,var(--specialty-laryngology)_68%,#db2777_32%))]',
      ctaClass: 'text-[color-mix(in_oklab,var(--specialty-laryngology)_78%,var(--app-text)_22%)]',
    };
  }

  return {
    chipClass: 'specialty-glow-otology',
    cardClass: 'border-[color-mix(in_oklab,var(--specialty-otology)_36%,var(--app-border)_64%)] bg-[linear-gradient(180deg,color-mix(in_oklab,var(--specialty-otology)_14%,var(--app-surface)_86%)_0%,color-mix(in_oklab,var(--specialty-otology)_24%,var(--app-surface-alt)_76%)_100%)]',
    stripeClass: 'bg-[linear-gradient(90deg,color-mix(in_oklab,var(--specialty-otology)_84%,#f97316_16%),color-mix(in_oklab,var(--specialty-otology)_68%,#f59e0b_32%))]',
    ctaClass: 'text-[color-mix(in_oklab,var(--specialty-otology)_78%,var(--app-text)_22%)]',
  };
};
...
export default function HomePage() {
  ...
  const ensureWelcomeSupportChat = useCallback(async (): Promise<string> => {
  ...
  }, [user, isAdmin, ensureWelcomeSupportChat, loadSupportMessagesByChatId]);
  ...
  }, [user, activeSupportChatId, isAdmin, isChatOpen, ensureWelcomeSupportChat, loadSupportMessagesByChatId]);
  ...
  }, [isChatOpen, user, isAdmin, supportChats.length, ensureWelcomeSupportChat, loadSupportMessagesByChatId]);
              ...
              >
                {highlights.map((item) => (
                  <motion.div
                    ...
                    className="rounded-2xl backdrop-blur-md px-4 py-3 border border-(--hero-panel-border) bg-(--hero-panel-bg)"
                  >
                    <p className="inline-flex items-center gap-1 text-xs uppercase tracking-[0.14em] text-(--hero-body)">
                      <item.icon className="h-3.5 w-3.5" />
                      {item.label}
                    </p>
                    <p className="text-sm md:text-base font-semibold mt-1 text-(--hero-title)">{item.value}</p>
                  </motion.div>
                ))}
              </motion.div>
            </div>
          ...
          <div className="container mx-auto px-4">
            ...
            >
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-5">
                ...
                <span className="text-sm font-semibold text-[color-mix(in_oklab,var(--app-accent)_76%,var(--app-text)_24%)]">
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-video overflow-hidden bg-slate-900">
                        ...
                        />
                        <div className={`absolute inset-x-0 top-0 h-1 ${theme.stripeClass}`} />
                        <div className="absolute inset-0 bg-linear-to-t from-black/60 via-black/15 to-transparent" />
                        <div className={`absolute top-3 left-3 inline-flex items-center rounded-full border border-white/35 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${theme.chipClass}`}>
                      ...
                      <div className={`px-4 pb-4 text-xs font-semibold ${theme.ctaClass}`}>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-16 items-center">
            ...
            <div className="relative">
                ...
                >
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="w-full rounded-full h-2 relative bg-(--demo-video-track-bg)">
                        <div className="h-2 w-full rounded-full bg-(--demo-video-track-fill)" />
                        <input
                          type="range"
                          min={0}
                          max={100}
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 min-w-33">
                          <button
                            ...
                            className="h-8 w-8 rounded-full border flex items-center justify-center transition-colors border-[color-mix(in_oklab,var(--demo-video-card-border)_78%,transparent)] bg-[color-mix(in_oklab,var(--demo-video-card-bg)_82%,transparent)] text-(--demo-video-time)"
                          >
              ...
              <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob bg-[color-mix(in_oklab,var(--app-accent)_30%,white_70%)]" />
              <div className="absolute -bottom-6 -left-6 w-32 h-32 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000 bg-[color-mix(in_oklab,var(--app-accent)_42%,white_58%)]" />
            </div>
          </div>
        </div>
      </div>
          ...
          >
            {journey.map((node) => (
              <motion.div
                key={node.step}
                variants={itemRise}
                className="rounded-2xl border p-5 border-[color-mix(in_oklab,var(--app-accent)_24%,var(--app-border)_76%)] bg-[linear-gradient(180deg,color-mix(in_oklab,var(--app-surface)_96%,white_4%)_0%,color-mix(in_oklab,var(--app-surface-alt)_84%,var(--app-accent)_16%)_100%)]"
              >
        ...
        <div className="py-20">
          <div className="container mx-auto px-4">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.55 }}
              className="rounded-3xl p-8 md:p-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 text-(--hero-title) border border-[color-mix(in_oklab,var(--hero-chip-border)_72%,var(--app-border)_28%)] bg-[linear-gradient(110deg,var(--hero-bg-start)_0%,color-mix(in_oklab,var(--hero-bg-end)_82%,var(--app-accent)_18%)_100%)]"
            >
              <motion.div
                initial={{ opacity: 0, x: -18 }}
                whileInView={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <p className="text-(--hero-body)">Commencez gratuitement puis activez le parcours adapté à votre rythme.</p>
              </motion.div>
              <motion.div
                initial={{ opacity: 0, x: 18 }}
                whileInView={{ opacity: 1, x: 0 }}
              ...
              >
                <Link
                  href="/sign-up"
                  className="inline-flex items-center justify-center rounded-xl px-6 py-3 font-semibold transition-colors bg-(--app-accent) text-(--app-accent-contrast)"
                >
                  Créer mon compte
                </Link>
                <Link
                  href="/pricing"
                  className="inline-flex items-center justify-center rounded-xl px-6 py-3 font-semibold transition-colors text-(--hero-title) border border-(--hero-chip-border) bg-(--hero-ghost-bg)"
                >
                  Voir les tarifs
                </Link>
              </motion.div>
      ...
      {!isAdmin ? (
        <div className="fixed bottom-5 right-5 z-70">
          ...
          {isChatOpen ? (
            ...
            {!isAuthReady ? (
              <div className="p-4 text-sm text-(--app-muted)">Chargement...</div>
            ) : !user ? (
              <div className="p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  <Link href="/sign-in" className="rounded-xl px-4 py-2 text-sm font-semibold bg-(--app-accent) text-(--app-accent-contrast)">
                    Se connecter
                  </Link>
                  <Link href="/sign-up" className="rounded-xl px-4 py-2 text-sm font-semibold border border-(--app-border) text-(--app-text)">
                    Creer un compte
                  </Link>
                </div>
              </div>
